...
imshow(T1);
T2 = uint8(255*rand(20,40,3));
figure,imshow(T2); % al poner figure delante, abrira otra ventana para la previsualización
clear,clc,close all;

% Leer imagenes
I=imread('X.jpg'); % Imagen en escala de grises - ver variable en workspace (el asume que la imagen es de color, le asigna a RGB el mismo color porque asume que esta en el mismo color con la información normalizada valor/256 )
